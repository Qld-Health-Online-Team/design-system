...
// $ (jQuery + jquery-validation) and Squiz_Matrix_API are provided by the host
// page (Squiz Matrix) — they are not bundled dependencies.
...
const isFileValid = function (file, input_field_settings) {
  ...
  const illegalFileNameCharacters =
    /[<>:"/\\|?*\x00-\x1F]|(<script.*?>|<\/script>|<.*?>|javascript:|<html.*?>|<\/html>)/i;

  // If the file name contains illegal characters
  if (illegalFileNameCharacters.test(fileName)) {
    console.error(
      "Unsupported characters in file name. Only use letters, numbers, space, and special characters: -_(’",
    );
  ...
  if (
    ...
    return (
      "Filename: '" +
      fileName +
      "' already in use. Please rename file before trying again."
    );
  }
 ...
 * @memberof module:fileUploads
 */
fileUploads.init = function (root = document) {
  const iconDataPathElement = root.querySelector(
    "form.qld__form--validate[data-path]",
  );
  fileUploads.iconDataPath = iconDataPathElement
    ? iconDataPathElement.getAttribute("data-path")
    : "";

  // Store all file input fields in inputs property
  let $file_inputs = Array.from(
    root.querySelectorAll("input[type=file].qld__file-input"),
  );

  // Bind listeners, and initialise settings object for each input field
  for (let i = 0; i < $file_inputs.length; i++) {
    const $input = $file_inputs[i];
    const $dropzone_element = $input
      .closest(".qld__form-file-wrapper")
      .querySelector(".qld__form-file-dropzone");
    const $file_list = $input
      .closest(".qld__form-file-wrapper")
      .querySelector(".qld__form-file-preview");
    ...
    fileUploads.inputs[$input.id] = {
      ...
      files:
        $input.dataset["files"] !== ""
          ? JSON.parse($input.dataset["files"])
          : [],
    ...
    if (
      fileUploads.inputs[$input.id].js_api_key !== undefined &&
      fileUploads.inputs[$input.id].input_element.hasAttribute("required")
    ) {
    ...
    if (
      fileUploads.inputs[$input.id].js_api_key !== undefined &&
      fileUploads.inputs[$input.id].create_location !== undefined
    ) {
      fileUploads.inputs[$input.id].jsApi = jsApi(
        fileUploads.inputs[$input.id].js_api_key,
      );
    }
  }
};
    ...
    $.validator.addMethod(
      ...
      function (value, element) {
        ...
        if ($(element).data("interacted")) {
          ...
          return (
            element.dataset["files"] !== "[]" && element.dataset["files"] !== ""
          );
        } else {
...
const deleteFile = async function (input_field_settings, fileId, $fileInfo) {
  ...
  // Validate file input (jQuery validation is provided by the host page; skip when absent, e.g. Storybook)
  if (window.$?.validator) {
 ...
 * @memberof module:fileUploads
 */
const deleteAssetFromMatrix = async function (
  fileId,
  $fileInfo,
  ...
  try {
    ...
    $loading.querySelector(".qld__form-file-info-status").innerText =
      "Deleting...";
...
const handleFiles = async function (files, input_field_settings) {
  ...
  for (const element of files) {
    ...
    if (isValid === true) {
      ...
      if (usingJsApi) {
        promiseArray.push(
          uploadFileJsApi(sanitisedFile, $fileInfo, input_field_settings),
        );
      } else {
...
const simulateFileUpload = function (file, $fileInfo) {
  return new Promise(function (resolve, reject) {
    ...
    let newText = success
      .querySelector(".qld__form-file-info-status")
      .textContent.replace("successful", "complete");
    ...
    for (var i = 0; i < text.childNodes.length; i++) {
      ...
      if (
        ...
        node.textContent.includes("successful")
      ) {
...
const uploadFileJsApi = async function (file, $fileInfo, input_field_settings) {
  ...
  try {
    ...
    if (!newFileAsset.hasOwnProperty("error")) {
      ...
      let updatedFileAsset = await updateFileContents(
        newFileAsset,
        fileContent,
        input_field_settings,
      );
      ...
      if (!updatedFileAsset.hasOwnProperty("error")) {
        ...
        let newFileInfo = await jsApi.getGeneral({
  ...
  try {
    ...
    if (!newFile.hasOwnProperty("error")) {
      ...
      if (newFile.errorDetails.length > 0) {
        let err = {};
        err.error = newFile.errorDetails[0].includes("web path already exists")
          ? "Filename already in use. Please rename file before trying again."
          : newFile.errorDetails[0];
        throw err;
      }
    }
  } catch (error) {
 ...
 * @memberof module:fileUploads
 */
const updateFileContents = async function (
  asset,
  fileContent,
  input_field_settings,
) {
      ...
      for (let file of files) {
        try {
          ...
          if (currentFile.error === undefined) {
            ...
            throw new Error(
              "displayExistingFiles: Could not retrieve asset attributes.",
            );
          }
        } catch (error) {
          console.log(error);
 ...
 * @memberof module:fileUploads
 */
const setFilesDataAttribute = function (
  input_field_settings,
  newFileInfo = null,
) {
