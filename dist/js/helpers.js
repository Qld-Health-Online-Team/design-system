...
Handlebars.registerHelper('generateDates', function (dates) {
  ...
  function convertIntoRange(dateArray, type, hidden) {
    ...
    if (dateArray.length > 0) {
      ...
      if (type == "upcoming") {
        html +=
          "<div>No upcoming dates are scheduled at this stage. Please check again later.</div>";
      } else {
...
Handlebars.registerHelper('isPage', function (pageType, options) {
  if (
    pageType == "page_standard" ||
    pageType == "page_redirect" ||
    pageType == "page_custom_form" ||
    pageType == "page_decision_tree"
  ) {
    return options.fn(this);
  } else {
    return options.inverse(this);
...
Handlebars.registerHelper('listAZ', function (items, letterHeading, url, options) {
  ...
  for (var i = 0; i < letters.length; i++) {
    ...
    html +=
      "<" +
      letterHeading +
      ' class="qld__a-z_listing__list__item__header"><span id="' +
      letters[i] +
      '">' +
      letters[i] +
      "</span></" +
    ...
    for (var k = 0; k < services[letters[i]].length; k++) {
      html +=
        '<li class="qld__a-z_listing__list__item__services__item"><a class="qld__a-z_listing__list__item__services__item__link" href="./?a=' +
        services[letters[i]][k].id +
        '"><span>' +
        services[letters[i]][k].name +
        "</span></a></li>";
    }
    html += "</ul>";
  }
  ...
  for (let selectValue in splitSelectValues) {
    if (splitSelectValues.hasOwnProperty(selectValue)) {
      output =
        output +
        options.fn({
          key: splitSelectValues[selectValue].replace(/ /g, "_"),
          value:
            splitSelectValues[selectValue].charAt(0).toUpperCase() +
            splitSelectValues[selectValue].slice(1),
        });
    }
  }
...
Handlebars.registerHelper('renderEncoded', function (string) {
  if (!string) return "";
  let cleaned = string;
  let prev;
