...
function checkAccordions(button) {
  if (
    button.parentNode
      .querySelector("." + navLinkClass)
      .classList.contains(navLinkClass + "--open")
  ) {
    openNavLink(button);
  } else {
    closeNavLink(button);
...
function openNavLink(button) {
  ...
  button.parentNode
    .querySelector("." + navLinkClass)
    .classList.add(navLinkClass + "--open");
  // Open the child element
  button.parentNode
    .querySelector("." + accordionChildClass)
    .classList.replace(accordionClosedClass, accordionOpenClass);
}
...
function closeNavLink(button) {
  ...
  button.parentNode
    .querySelector("." + navLinkClass)
    .classList.remove(navLinkClass + "--open");
  // Close the child element
  button.parentNode
    .querySelector("." + accordionChildClass)
    .classList.replace(accordionOpenClass, accordionClosedClass);
}
