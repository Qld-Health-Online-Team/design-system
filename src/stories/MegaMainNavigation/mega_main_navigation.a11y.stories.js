...
export const ArrowKeyNavigation = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const toggle = canvas.getByRole("button", {
      name: /toggle navigation, conditions/i,
    });
    ...
    await waitFor(() => {
      if (subMenu.style.display === "none")
        throw new Error("submenu still hidden");
    });
...
export const MutualExclusion = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const servicesToggle = canvas.getByRole("button", {
      name: /toggle navigation, services/i,
    });
    const conditionsToggle = canvas.getByRole("button", {
      name: /toggle navigation, conditions/i,
    });
