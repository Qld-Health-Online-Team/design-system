...
export default function initVideoPlayer(document = document) {
  ...
  const accordionTitles = document.querySelectorAll(
    ".qld__body .qld__video__player--wrapper .qld__accordion__title",
  );
...
function toggleButtonState(accordionTitle) {
  ...
  const isCurrentlyOpen = accordionTitle.classList.contains(
    "qld__accordion--open",
  );
  buttonSpan.textContent = isCurrentlyOpen
    ? "Show transcript"
    : "Hide transcript";
}
