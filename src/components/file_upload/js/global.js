(function () {
  ...
  fileUploads.init = function () {
    const iconDataPathElement = document.querySelector(
    ...
    let $file_inputs = QLD.utils.listToArray(
      document.querySelectorAll("input[type=file].qld__file-input"),
    );
  ...
  const addListeners = function (input_field_settings) {
    ...
    const $dropArea = $fileInputWrapper.querySelector(
  ...
  const deleteAssetFromMatrix = async function (
    ...
    try {
      const name = $fileInfo.querySelector(
   ...
   * @memberof module:fileUploads
   */
  const uploadFileJsApi = async function (
    file,
    $fileInfo,
    input_field_settings,
  ) {
    ...
    try {
      ...
      if (!newFile.hasOwnProperty("error")) {
        ...
        if (newFile.errorDetails.length > 0) {
          let err = {};
          err.error = newFile.errorDetails[0].includes(
