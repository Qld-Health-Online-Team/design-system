...
function getTheElements(root = document, resized = false) {
  const bannerBreadCrumbsAll = root.querySelectorAll(
    "nav.qld__banner__breadcrumbs--desktop",
  );

  const bodyBreadCrumbsAll = root.querySelectorAll(
  ...
  if (bannerBreadCrumb) {
    ...
    if (resized) {
      ...
      bannerBreadCrumb
        .querySelector("ol.qld__link-list")
        .replaceChildren(...originalList.children);
    }
...
function checkChevrons(root = document) {
  const component = root.querySelector(
    ".qld__breadcrumbs .qld__link-list.qld__breadcrumbs__list--desktop",
  );
  ...
  if (getTheElements(root)) {
    ...
    if (breadCrumbsUlLis.length > 2 && breadCrumbsUlLis[0].offsetHeight > 0) {
      ...
      if (breadcrumbLisLength > 5) {
        ...
        while (
          breadCrumbsUl.offsetHeight > breadCrumbsUlLis[0].offsetHeight * 1.9 &&
