export const SelectBox = ({ id, isFilled, isRequired, isDisabled, label, hintText, state, succcessMessage, errorMessage, defaultOption, options }) => {
    let stateMessage = "";
    let stateClass = "";
    ...
    return `
        <fieldset ${isFilled ? "class='qld__form-style-filled'" : ""}>
            ...
            ${stateMessage}
            <select id="${id}" class="qld__select-control ${stateClass}" ${isDisabled ? "disabled" : ""} name="${id}" aria-describedby="${id}-hint ${id}-state-message">
                ${optionsMapped}
            </select>
        </fieldset>
    `;
