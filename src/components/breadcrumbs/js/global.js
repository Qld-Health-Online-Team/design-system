...
let originalBreadCrumbUl = null;

function getTheElements(root = document, resized = false) {
    const bannerBreadCrumbsAll = root.querySelectorAll("nav.qld__banner__breadcrumbs--desktop");

    const bodyBreadCrumbsAll = root.querySelectorAll("section.qld__body--breadcrumb nav.qld__breadcrumbs");
...
function checkChevrons(root = document) {
    const component = root.querySelector(".qld__breadcrumbs .qld__link-list.qld__breadcrumbs__list--desktop");
...
/**
 * Initialise the breadcrumb overflow behaviour.
 *
 * @param {Document|Element} root - The root element to search for breadcrumbs within.
 * @param {boolean} resized - When true, reset the breadcrumb list to its
 *   original markup before recalculating (used on window resize).
 */
export default function initBreadcrumb(root = document, resized = false) {
    if (resized) {
        getTheElements(root, true);
    }

    if (getTheElements(root)) {
        const { breadCrumbsUl } = getTheElements(root);
