...
function renderTextInput({
  ...
  return (
    ...
    `</label>` +
    (hint
      ? `<span class="qld__hint-text" id="${id}-hint">${hint}</span>`
      : "") +
    feedbackHtml +
...
function renderForm({ filled, showErrors }) {
  const style = filled
    ? ' class="qld__body qld__form qld__form-style-filled"'
    : ' class="qld__body qld__form"';
      ...
      inputs: [
        {
          id: "consent-1",
          label:
            "I consent to my information being used to process this enquiry",
          checked: false,
          disabled: false,
          inError: showErrors,
          inSuccess: false,
